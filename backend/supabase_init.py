#!/usr/bin/env python3
"""
Initialize Supabase PostgreSQL database with PropBase schema
...
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "postgresql://postgres.ktwtijwgsvvvvylspcmk:9?!hv4k%smsRp@g@aws-1-ap-southeast-1.pooler.supabase.com:6543/postgres"
)

def parse_database_url(url: str) -> dict:
    """Parse DATABASE_URL handling special characters in password (like @)"""
    # Remove protocol
    if url.startswith('postgresql://'):
        url_part = url[len('postgresql://'):]
    elif url.startswith('postgres://'):
        url_part = url[len('postgres://'):]
    else:
        url_part = url
    
    # Split from the end - last @ is host separator
    at_idx = url_part.rfind('@')
    user_pass = url_part[:at_idx]
    host_db = url_part[at_idx+1:]
    ...
    db = host_db[slash_idx+1:]
    
    port_idx = host_port.find(':')
...
def init_schema():
    ...
    CREATE TABLE IF NOT EXISTS collection_items (
        ...
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        UNIQUE(collection_id, project_id, unit_id)
    );
